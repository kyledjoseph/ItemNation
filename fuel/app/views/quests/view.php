...
<div class="box marg-top">
	...
	<div class="row">
		<div class="col-12">
			<div id="chat-wrapper" class="chat">
				<?php if (empty($quest_messages)): ?>
				<div class="message">No messages</div>
			<?php else: ?>
			<?php foreach ($quest_messages as $message): ?>
                        <?php echo View::forge('quests/templates/message', array('message' => $message))->render(); ?>
                        <?php endforeach; ?>
	<?php endif; ?>
</div>
</div>
	...
	<div class="row">
		<div class="col-12">
			<?= Form::open(array('action' => $quest->url('message'), 'class' => 'mx-chat-form', 'ajax-action' => 'append', 'ajax-target' => 'chat-wrapper', 'ajax-reset' => '1')) ?>
			<div class="input-group">
				<?php if (isset($user)): ?>
